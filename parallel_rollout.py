...
from denoisers import GenCastDenoiser
from graphcast import denoiser, gencast, graphcast, xarray_jax
...
def step(
    ...
    """
    Draw next state x^{k+1}_{i} from p(x^{k+1} | x^{k}_{i})
    Input(s)
        - inputs (xarray.Dataset): previous state x^{k}_{i} with dimension (batch=1, time=2, lat=181, lon=360, levels=13)
        - target_template (xarray.Dataset): target template with dimension (batch=1, time=1, lat=181, lon=360, levels=13)
        - forcings (xarray.Dataset): forcing terms used the GenCast denoiser with dimension (batch=1, time=1, lat=181, lon=360, levels=13)
        - task_config (graphcast.TaskConfig)
        - denoiser_config (denoiser.DenoiserArchitectureConfig)
        - noise_encoder_config (denoiser.NoiseEncoderConfig)
    ...
    Returns
        - next_state (xarray.Dataset): next state x^{k+1}_{i} with dimension (batch=1, time=2, lat=181, lon=360, levels=13)
    """
